...
function Account() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setOrders(await fetchOrdersForEmail(user.email));
        log.info("account:orders:loaded");
      } catch (err) {
        log.exception("account:orders:failed", err);
      } finally {
  ...
  return (
    <div className="container-page py-12 md:py-16">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <button
          onClick={() => {
            log.event("account:signout:click");
            signOut();
            navigate({ to: "/" });
          }}
            ...
            {orders.map((o, i) => (
              ...
              >
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs text-accent uppercase tracking-widest mt-0.5">
                    {o.status}
                  </div>
                </div>
              </li>
