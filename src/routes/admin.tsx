  ...
  return (
    <div className="card-surface p-6 flex items-start gap-4">
      ...
      <div className="min-w-0">
        ...
        <p className="mt-1 text-2xl font-display font-semibold leading-none">
          {value}
        </p>
...
function Admin() {
  ...
  const stats = useMemo(() => {
    const revenue = orders
      .filter((o) =>
        ["paid", "processing", "shipped", "fulfilled"].includes(o.status),
      )
  ...
  function patchShipping(field: keyof ShippingDetails, value: string) {
    setOrderDraft((d) => ({
      ...d,
      shipping: { ...((d.shipping ?? {}) as ShippingDetails), [field]: value },
    }));
  }
      ...
      <header className="border-b bg-card/80 backdrop-blur-sm sticky top-0 z-30">
        <div className="container-page h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <a href="/" className="btn-outline !py-1.5 !px-3 text-xs">
              ← Store
            </a>
          </div>
        </div>
              ...
              {navItems.map((item) => (
                ...
                >
                  ...
                  {item.count !== undefined && (
                    <span
                      className={`text-xs px-1.5 py-0.5 rounded-md ${tab === item.id ? "bg-white/20" : "bg-muted"}`}
                    >
                      {item.count}
                    </span>
                  )}
            ...
            {loadingData && (
              <div className="space-y-4">
                {[...Array(3)].map((_, i) => (
                  <div
                    key={i}
                    className="card-surface h-24 animate-pulse opacity-50"
                  />
                ))}
              </div>
            )}
                          ...
                          <table className="w-full text-sm">
                            <thead>
                              <tr className="border-b">
                                <th className="text-left px-5 py-3 text-xs uppercase tracking-widest text-muted-foreground font-medium">
                                  Reference
                                </th>
                                <th className="text-left px-5 py-3 text-xs uppercase tracking-widest text-muted-foreground font-medium hidden sm:table-cell">
                                  Email
                                </th>
                                <th className="text-right px-5 py-3 text-xs uppercase tracking-widest text-muted-foreground font-medium">
                                  Amount
                                </th>
                                <th className="text-center px-5 py-3 text-xs uppercase tracking-widest text-muted-foreground font-medium">
                                  Status
                                </th>
                              </tr>
                            </thead>
                        ...
                        <div className="grid sm:grid-cols-2 gap-3">
                          {products
                            ...
                            .map((p) => (
                              <div
                                key={p._id ?? p.id}
                                className="card-surface p-4 flex items-center gap-3 border-l-4 border-l-destructive"
                              >
                                <img
                                  src={p.image}
                                  alt={p.name}
                ...
                {tab === "products" && (
                  <div className="space-y-6">
                    ...
                    {showForm && (
                      ...
                      >
                        <div
                          className="card-surface w-full max-w-lg shadow-2xl p-7 max-h-[90vh] overflow-y-auto"
                        ...
                        >
                          <div className="flex items-center justify-between mb-5">
                            ...
                            <button
                              onClick={() => setShowForm(false)}
                              className="w-8 h-8 rounded-full border flex items-center justify-center text-muted-foreground hover:text-foreground transition"
                            >
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              ...
                              <div className="col-span-2">
                                ...
                                {draft.image && (
                                  <img
                                    src={draft.image}
                                    className="mt-2 w-full h-36 object-cover rounded-lg bg-secondary"
                                    alt="preview"
                                  />
                                )}
                              </div>
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="border-b bg-muted/30">
                              ...
                              <th className="text-left px-5 py-3 text-xs uppercase tracking-widest text-muted-foreground font-medium hidden md:table-cell">
                                Category
                              </th>
                              <th className="text-right px-5 py-3 text-xs uppercase tracking-widest text-muted-foreground font-medium">
                                Price
                              </th>
                              <th className="text-right px-5 py-3 text-xs uppercase tracking-widest text-muted-foreground font-medium hidden sm:table-cell">
                                Stock
                              </th>
                          ...
                          <tbody>
                            {products.map((p) => (
                              <tr
                                key={p._id ?? p.id}
                                className="border-b last:border-0 hover:bg-muted/20 transition group"
                              >
                                ...
                                <td className="px-5 py-3 text-right hidden sm:table-cell">
                                  <span
                                    className={`font-medium ${p.stock <= 3 ? "text-destructive" : ""}`}
                                  >
                                    {p.stock}
                                  </span>
                                </td>
                ...
                {tab === "orders" && (
                  <div className="space-y-6">
                    ...
                    {filteredOrders.length === 0 ? (
                      <div className="card-surface p-16 text-center text-muted-foreground text-sm">
                              ...
                              <div className="px-5 py-4 flex flex-wrap items-start justify-between gap-4 bg-muted/20">
                                ...
                                <div className="flex items-center gap-3">
                                  <StatusBadge
                                    status={
                                      editing
                                        ? (orderDraft.status ?? order.status)
                                        : order.status
                                    }
                                  />
                                ...
                                <div className="p-5 border-t">
                                  <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
                                    ...
                                    <Field
                                      label="Name"
                                      value={shipping?.name ?? ""}
                                      onChange={(v) => patchShipping("name", v)}
                                    />
                                    <Field
                                      label="Phone"
                                      value={shipping?.phone ?? ""}
                                      onChange={(v) =>
                                        patchShipping("phone", v)
                                      }
                                    />
                                    ...
                                    <Field
                                      label="City"
                                      value={shipping?.city ?? ""}
                                      onChange={(v) => patchShipping("city", v)}
                                    />
                                    <Field
                                      label="State"
                                      value={shipping?.state ?? ""}
                                      onChange={(v) =>
                                        patchShipping("state", v)
                                      }
                                    />
                                  </div>
